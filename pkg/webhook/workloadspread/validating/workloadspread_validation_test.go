...
func TestValidateWorkloadSpreadCreate(t *testing.T) {
	...
	errorCases := []struct {
		...
		{
			name: "subset-a's patch is not a valid podTemplate",
			getWorkloadSpread: func() *appsv1alpha1.WorkloadSpread {
				patch := runtime.RawExtension{
					Raw: []byte(`{"Kind":"Job",metadata":{"annotations":{"subset":"subset-a"}}}`),
				}
				workloadSpread := workloadSpreadDemo.DeepCopy()
				workloadSpread.Spec.Subsets[0].Patch = patch
				return workloadSpread
			},
			errorSuffix: "spec.subsets[0].patch",
		},
		{
			name: "subset-a's patch is not a valid podTemplate",
			getWorkloadSpread: func() *appsv1alpha1.WorkloadSpread {
		...
		{
			name: "patch status is not allowed",
			getWorkloadSpread: func() *appsv1alpha1.WorkloadSpread {
				patch := runtime.RawExtension{
					Raw: []byte(`{"status":{"phase":"running"}}`),
				}
				workloadSpread := workloadSpreadDemo.DeepCopy()
				workloadSpread.Spec.Subsets[0].Patch = patch
				return workloadSpread
		...
		{
			name: "patch image is not allowed",
			getWorkloadSpread: func() *appsv1alpha1.WorkloadSpread {
				patch := runtime.RawExtension{
					Raw: []byte(`{"spec":{"containers":[{"name":"main","image":"busybox:latest"}]}}`),
				}
				workloadSpread := workloadSpreadDemo.DeepCopy()
				workloadSpread.Spec.Subsets[0].Patch = patch
				return workloadSpread
