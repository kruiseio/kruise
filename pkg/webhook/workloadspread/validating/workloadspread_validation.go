...
func validateWorkloadSpreadSubsetPatch(patch runtime.RawExtension, fldPath *field.Path) field.ErrorList {
	...
	if err := decoder.DecodeRaw(patch, obj); err != nil {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("patch"), patch, "the patch is not a valid podTemplate, can't be decoded a podTemplate"))
		return allErrs
	}

	if obj.TypeMeta.Kind != "" && obj.APIVersion != "" {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("patch"), patch, "the patch is not a valid podTemplate, kind and apiVersion is not allowed to patch"))
		return allErrs
	}
	if !reflect.DeepEqual(obj.Status, v1.PodStatus{}) {
		allErrs = append(allErrs, field.Invalid(fldPath.Child("patch"), patch, "the patch is not a valid podTemplate, status is not allowed to patch"))
		return allErrs
	}
...
func validateWorkloadSpreadSubsets(subsets []appsv1alpha1.WorkloadSpreadSubset, fldPath *field.Path) field.ErrorList {
	...
	for i, subset := range subsets {
		...
		// validate patch
		allErrs = append(allErrs, validateWorkloadSpreadSubsetPatch(subsets[i].Patch, fldPath.Index(i))...)
